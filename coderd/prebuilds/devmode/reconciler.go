...
// DevStoreReconciler is the development version of the prebuilds reconciler.
// It provides full prebuild pool management without license checks.
type DevStoreReconciler struct {
	...
	metrics *DevMetricsCollector
	// Operational metrics.
	reconciliationDuration prometheus.Histogram
}

var _ prebuilds.ReconciliationOrchestrator = &DevStoreReconciler{}

type devDeprovisionMode int

const (
	devDeprovisionModeNormal devDeprovisionMode = iota
	devDeprovisionModeOrphan
)

func (d devDeprovisionMode) String() string {
	switch d {
	case devDeprovisionModeOrphan:
		return "orphan"
	case devDeprovisionModeNormal:
		return "normal"
	default:
		return "unknown"
	}
...
func NewDevStoreReconciler(store database.Store,
	ps pubsub.Pubsub,
	fileCache *files.Cache,
	cfg codersdk.PrebuildsConfig,
	logger slog.Logger,
	...
	if registerer != nil {
		reconciler.metrics = NewDevMetricsCollector(store, logger, reconciler)
...
func (c *DevStoreReconciler) Run(ctx context.Context) {
...
func (c *DevStoreReconciler) Stop(ctx context.Context, cause error) {
	...
	if c.metrics != nil && c.registerer != nil {
		if !c.registerer.Unregister(c.metrics) {
			...
			// feature again. If the metrics cannot be registered, it'll log an error from NewDevStoreReconciler.
...
func (c *DevStoreReconciler) ReconcileAll(ctx context.Context) (stats prebuilds.ReconcileStats, err error) {
	...
	err = c.WithReconciliationLock(ctx, logger, func(ctx context.Context, db database.Store) error {
		...
		membershipReconciler := NewDevMembershipReconciler(c.store, c.clock, logger)
...
func (c *DevStoreReconciler) reportHardLimitedPresets(snapshot *prebuilds.GlobalSnapshot) {
	...
	presetsMap := make(map[devHardLimitedPresetKey][]database.GetTemplatePresetsWithPrebuildsRow)
	for _, preset := range snapshot.Presets {
		key := devHardLimitedPresetKey{
...
func (c *DevStoreReconciler) SnapshotState(ctx context.Context, store database.Store) (*prebuilds.GlobalSnapshot, error) {
...
func (c *DevStoreReconciler) ReconcilePreset(ctx context.Context, ps prebuilds.PresetSnapshot) error {
...
func (c *DevStoreReconciler) CalculateActions(ctx context.Context, snapshot prebuilds.PresetSnapshot) ([]*prebuilds.ReconciliationActions, error) {
	if ctx.Err() != nil {
		return nil, ctx.Err()
	}
...
func (c *DevStoreReconciler) WithReconciliationLock(
	ctx context.Context,
	logger slog.Logger,
...
func (c *DevStoreReconciler) executeReconciliationAction(ctx context.Context, logger slog.Logger, ps prebuilds.PresetSnapshot, action *prebuilds.ReconciliationActions) error {
...
func (c *DevStoreReconciler) createPrebuiltWorkspace(ctx context.Context, prebuiltWorkspaceID uuid.UUID, templateID uuid.UUID, presetID uuid.UUID) error {
	...
	err = c.store.InTx(func(db database.Store) error {
		...
		provisionerJob, err = c.provision(ctx, db, prebuiltWorkspaceID, template, presetID, database.WorkspaceTransitionStart, workspace, devDeprovisionModeNormal)
		return err
	}, &database.TxOptions{
		Isolation:    sql.LevelRepeatableRead,
		ReadOnly:     false,
...
//
// If mode is devDeprovisionModeOrphan, the builder will not send Terraform state to the provisioner.
...
func (c *DevStoreReconciler) provisionDelete(ctx context.Context, db database.Store, workspaceID uuid.UUID, templateID uuid.UUID, presetID uuid.UUID, mode devDeprovisionMode) (*database.ProvisionerJob, error) {
...
func (c *DevStoreReconciler) cancelAndOrphanDeletePendingPrebuilds(ctx context.Context, templateID uuid.UUID, templateVersionID uuid.UUID, presetID uuid.UUID) error {
	...
	err := c.store.InTx(func(db database.Store) error {
		...
		for _, job := range canceledJobs {
			provisionerJob, err := c.provisionDelete(ctx, db, job.WorkspaceID, job.TemplateID, presetID, devDeprovisionModeOrphan)
			if err != nil {
...
func (c *DevStoreReconciler) deletePrebuiltWorkspace(ctx context.Context, prebuiltWorkspaceID uuid.UUID, templateID uuid.UUID, presetID uuid.UUID) error {
	...
	err := c.store.InTx(func(db database.Store) (err error) {
		provisionerJob, err = c.provisionDelete(ctx, db, prebuiltWorkspaceID, templateID, presetID, devDeprovisionModeNormal)
		return err
	}, &database.TxOptions{
		Isolation:    sql.LevelRepeatableRead,
		ReadOnly:     false,
...
func (c *DevStoreReconciler) provision(
	ctx context.Context,
	db database.Store,
	...
	if transition == database.WorkspaceTransitionDelete && mode == devDeprovisionModeOrphan {
...
func (c *DevStoreReconciler) publishProvisionerJob(ctx context.Context, provisionerJob *database.ProvisionerJob, workspaceID uuid.UUID) {
	if provisionerJob == nil {
		return
	}
	select {
...
func (c *DevStoreReconciler) ForceMetricsUpdate(ctx context.Context) error {
	if c.metrics == nil {
		return nil
	}
...
func (c *DevStoreReconciler) TrackResourceReplacement(ctx context.Context, workspaceID, buildID uuid.UUID, replacements []*sdkproto.ResourceReplacement) {
...
func (c *DevStoreReconciler) trackResourceReplacement(ctx context.Context, workspaceID, buildID uuid.UUID, replacements []*sdkproto.ResourceReplacement) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
type DevPrebuildsSettings struct {
	ReconciliationPaused bool `json:"reconciliation_paused"`
}

func SetDevPrebuildsReconciliationPaused(ctx context.Context, db database.Store, paused bool) error {
	settings := DevPrebuildsSettings{
