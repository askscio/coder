...
// DevMembershipReconciler encapsulates the responsibility of ensuring that the prebuilds system user is a member of all
...
type DevMembershipReconciler struct {
	store  database.Store
	clock  quartz.Clock
	logger slog.Logger
}

func NewDevMembershipReconciler(store database.Store, clock quartz.Clock, logger slog.Logger) DevMembershipReconciler {
	return DevMembershipReconciler{
		store:  store,
		clock:  clock,
		logger: logger,
	}
...
func (s DevMembershipReconciler) ReconcileAll(ctx context.Context, userID uuid.UUID, groupName string) error {
